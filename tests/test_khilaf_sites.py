...
def _read(packed, hafs, alphabet, site, junction, selection):
    ...
    score = hafs.build(
        hafs.read(
            Script.UTHMANI, VerseRef(surah, ayah), words_of(packed, surah, ayah)
        ),
        selection=selection,
    ).score
    plan = BoundaryPlan(
...
def test_the_documented_default_is_taken(
    packed, hafs, alphabet, site, key, joined, stopped
) -> None:
    plain = VariantSelection()
    assert _read(packed, hafs, alphabet, site, Junction.JOIN, plain) == joined
    assert _read(packed, hafs, alphabet, site, Junction.STOP, plain) == stopped
...
def test_both_wajh_are_reachable(
    ...
    readings = {
        name: _read(
            packed, hafs, alphabet, site, junction,
...
def test_a_site_choice_outranks_one_made_for_every_site(
    ...
    assert _read(packed, hafs, alphabet, (89, 4, 3), Junction.STOP, both) == "jasrˤ"
    assert _read(
        packed, hafs, alphabet, (20, 77, 6), Junction.STOP,
...
def test_the_undisputed_junction_is_left_to_the_rule(
    ...
    assert _read(
        packed, hafs, alphabet, (12, 99, 10), Junction.JOIN, light
    ) == "misˤrˤaˤ"


def test_the_optional_yaa_is_read_both_ways(packed, hafs, alphabet) -> None:
    ...
    readings = {
        name: _read(
            packed, hafs, alphabet, (27, 36, 8), Junction.STOP,
            VariantSelection(
    ...
    assert _read(
        packed, hafs, alphabet, (27, 36, 8), Junction.STOP, VariantSelection()
    ) == readings["hadhf"]
    assert _read(
        packed, hafs, alphabet, (19, 30, 5), Junction.STOP, VariantSelection()
...
def test_the_optional_yaa_is_always_said_when_joined(
    ...
    for name in ("hadhf", "ithbat"):
        assert _read(
            packed, hafs, alphabet, (27, 36, 8), Junction.JOIN,
            VariantSelection(
                (Option(KhilafId.YAA_ITHBAT, name, site="ءaتaنiيa"),)
            ),
        ) == "ʔa:ta:nija"
...
def test_the_site_keys_reach_the_words_they_name(sources, hafs) -> None:
    ...
    for script in Script:
        for (surah, ayah), words in sources[script].items():
            score = hafs.build(
                hafs.read(script, VerseRef(surah, ayah), words)
            ).score
...
def test_the_vowel_khilaf_takes_its_default(packed, hafs, alphabet) -> None:
    ...
    plain = VariantSelection()
    assert _read(
        packed, hafs, alphabet, (30, 54, 5), Junction.STOP, plain
    ) == "dˤaˤʕf"


def test_both_vowels_are_reachable(packed, hafs, alphabet) -> None:
    readings = {
        name: _read(
            packed, hafs, alphabet, (30, 54, 5), Junction.STOP,
...
def test_the_two_scripts_agree_on_the_vowel_khilaf(sources, hafs) -> None:
    """The khilaf is what makes them agree: one script writes each wajh."""
    digests = {
        script: hafs.build(
            hafs.read(script, VerseRef(30, 54), sources[script][(30, 54)])
...
def test_the_imala_takes_its_documented_default(packed, hafs, alphabet) -> None:
    ...
    readings = {
        name: _read(
            packed, hafs, alphabet, (11, 41, 6), Junction.STOP,
            VariantSelection(
    ...
    assert _read(
        packed, hafs, alphabet, (11, 41, 6), Junction.STOP, VariantSelection()
    ) == readings["i"]


def test_both_scripts_reach_the_imala(sources, hafs, alphabet) -> None:
    ...
    for script in Script:
        score = hafs.build(
            hafs.read(script, VerseRef(11, 41), sources[script][(11, 41)])
        ).score
