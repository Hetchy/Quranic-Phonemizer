...
def test_a_quiescent_lam_merges_into_a_following_raa(ref, words, expected):
    ...
    assert "idgham_mutaqaribayn" in r.rules_on_char(first, "ل")
    assert "idgham_mutaqaribayn" in r.rules_on_sound(last, "rˤrˤ")
...
def test_a_quiescent_qaaf_merges_into_a_kaaf_inside_one_word(r):
    ...
    assert "idgham_mutaqaribayn" in r.rules_on_char(2, "ق")
    assert r.rules_on_sound(2, "kk") == {"idgham_mutaqaribayn"}
...
def test_a_stop_after_the_lam_undoes_the_merger_into_the_raa(r):
    ...
    assert "idgham_mutaqaribayn" not in r.rules_on_char(1, "ل")
