...
edge, takes no pairing and is dropped (row 13); the space before the word
carries the same edge and is kept (row 34). The elided seat produces no
rendered glyph rather than an empty one, and the dagger's expansion into a
written alif is row 29.
...
noon, and its seat goes by row 6; under plan B the fathatan becomes a fatha
(row 23) and the same seat stops being silent (row 31).
...
recited text drops the first (row 13) and keeps the other two (rows 34 and 33).
...
Word 1's waw and its round zero are rows 5 and 7; word 2's dagger is row 29;
word 3's fathatan becomes a fatha (row 23), its seat stops being silent (row
31) and is written as the carrier the long a takes (row 19).
...
Word 1 is rows 26 and 15 on the wasl seat; word 2 is row 18, the one insertion
whose sound the source text does write; word 3 is row 21.
...
Word 3 is 4.1a's idgham line, the haraka alone, and its seat goes by row 6.
Word 4 takes row 14: the rasm left the noon bare because an assimilation was
...
Word 1 is rows 26 and 15 on the wasl seat. The quiescent meems of words 2 and 5
go by row 9, and word 5 takes the shadda row 20 adds. Word 4's kasratan is
4.1a's idgham line and word 7's is row 22.
...
Word 2's qaf and its final meem go by row 9, and so does word 3's noon; the
hosts already carry their shadda, so row 20 adds nothing. Word 5 is row 22.
Under plan B word 3 loses its shadda to row 11.
...
Word 7 is 4.1a's idgham line and its seat goes by row 6. Word 9 is rows 1 and
24: the tanween goes and leaves no alif, and the taa marbuta is written as the
haa recitation says. Under plan B word 8 loses its shadda to row 11.
...
that haa: row 1 takes the tanween, row 24 substitutes the letter, and rows 14,
...
Word 2's noon goes by row 9 and the host already carries its shadda. Word 3 is
row 30, the silah mark written out as a haraka and a carrier. Word 4 is 4.1a's
izhar line, the haraka then a noon bearing a sukun, and its seat goes by row 6.
Word 5 is row 22.
...
The merger of word 9 is row 9 alone: the mushaf already doubles the host, so
row 20 has nothing to add. Word 10 is row 30. The sakt mark of word 11 is 4.7's
...
writes exactly as the rasm does. Word 9 is row 30 under plan A; under plan B
row 2 takes the silah mark and the sukun that replaces its kasra is covered by
no row. Word 10 is row 21.
...
row 16 writes the kasra itself. Word 13 takes rows 13, 20 and 29 together, and
word 14 rows 8 and 9.
...
insertion's own glyph, which is why row 16 writes a letter and not only a mark.
...
Word 4 is rows 8, 10 and 18: the wasl seat goes, the article lam goes, and the
long a the rasm has no carrier for is written. Word 5 is 4.7's unchanged case
for a joined iqlab. Word 7 is row 29 on the dagger and row 30 on the silah mark
under plan A; under plan B row 2 takes the mark and the sukun that replaces its
kasra is covered by no row. Word 8 is row 21.
...
| script - recited | draw مِيثَاقِهْ under plan B | **FINDING.** Row 2 removes ۦ, and the kasra becoming a sukun is covered by no row: row 21 wants a vowel short in its joined form, row 22 a nunation, row 23 a fathatan, row 14 a bare consonant |
...
Word 12 is row 30 under plan A; under plan B rows 2 and 3 take the mark and its
maddah together, and the sukun that replaces its kasra is covered by no row.
Word 13's noon goes by row 9 and
word 14 takes the shadda row 20 adds. Word 16 loses its carrier to row 12, word
17 its wasl seat to row 8, and the stop sign is row 33.
...
Word 10 is rows 26 and 15 on the wasl seat and row 10 on the article lam. Word
11's fathatan becomes a fatha and an alif is written where the rasm has no seat
for it to lengthen, which is row 17 beside row 23.
...
| script - recited | read the fathatan's two rendered rows | the fatha it becomes and the alif that was never written. Row 23 names a written seat this word does not have and row 17 is the one that supplies the alif, so the substitution is stated by neither alone |
...
form, so its seat is row 6; the second word's is short in both, so nothing but
the stop takes it and its seat is row 5. Both silence signs are row 7. This is
...
Word 1 is one glyph per letter name, so row 28 spells it as two names with a
...
shadda the host needs, so row 20 adds nothing; word 6 is row 22.
...
by row 6. Word 15 loses its carrier to row 12 under plan A and keeps it under
plan B. Word 16 under plan B is row 26 on the seat, row 15 for the haraka and
row 25 for the quiescent hamza. Word 17 is rows 29 and 2, and the sukun that
...
| script - rules | account for word 15's yaa | one field read, and a consumer that reached for the reclass finds a deletion: the yaa is written in one plan and not in the other, and the rule that removes it is `iltiqa_shortening` rather than row 32's reclass |
...
Word 6 is row 29 on a dagger whose vowel is not a long a: the expansion writes
...
row 29 read together with the unit's own quality. The maddah of word 7 stands
because the plan joins, which is the contrast row 3 is stated against, and its
stop sign is row 33. Word 10 is 4.1a's idgham line and word 11 already carries
the shadda.
...
spelling is the recited spelling. Word 7 is row 29 and word 8 is row 21.
...
Word 8 is row 29 on the dagger and row 30 on the silah mark, which the joining
...
and word 10 takes the shadda row 20 adds; word 10 is row 22.
...
| row 4, an iqlab when the plan stops | the Uthmani corpus text carries no small high meem, so no passage can show that mark present joined and absent stopped. E13 shows the noon and the tanween halves of the row |
| row 27, the imala respelling | the catalogue prints no row 27, so there is nothing to show. E19 writes the respelling as row 29 read together with the unit's own quality |
| row 32, the word-final role flip | no word in the set ends in a voweled yaa, waw or alif maqsura at a stop. E18 reaches for the row and finds a deletion instead |
...
Rows 4 and 27 are not printed in the catalogue at all, which is why the counts
in section 4.8 of that document exceed the rows above them. Where a rule or a
row needed a transformation the catalogue does not carry, the example states
the transformation and marks it, rather than inventing a row: the sukun on a
