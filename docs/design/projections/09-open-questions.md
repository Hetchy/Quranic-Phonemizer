...
**Nothing is open.** The last four entries closed together: `tafkheem`'s
trigger and `MergedInto.by` corrected the documents, and the tatweel seat and
`replace()` corrected the package. The two rounds overlapped on a fifth of what
they found, so an empty register is evidence that what was found is answered,
not that nothing else is there.
