...
1. What did the four legacy projections actually publish, and which consumer
   read which field?
...
letter_phoneme_mapping,tajweed_mapping,silent,tajweed_rule}.py`; the branch
...
They are four **traversals of the same join**, computed four times, over a model
