...
- the mini seen of a seen/saad khilaf is a unit wherever the script writes it.
  It presents the seen realization while the base saad remains the sound owner
  and alignment host in both faces; selecting saad marks the mini seen as
  variant-silent without removing its column. Whether the
...
- the one silence no rule accounts for is a letter the script writes and the
  reading does not read -- the alif of قَالُوا۟, or a selected seen/saad mini
  seen -- which after 11.1 has no occurrence to name. It takes the literal
  orthographic or variant silence respectively. Where a selected variant
  decided it, the column's
...
- back: a silent unit joins the group of the unit before it. Silent alif, waw,
  ...
  and a variant-silent mini seen, which rides the base letter that owns the
  realization in both faces.
- forward: a silent unit at a word start joins the group of the next sounding
  ...
  wasl or silent long vowel of an iltiqa.
...
in a shared vocabulary. It does not need one. It is a letter that can present
the base-owned sound or be variant-silent, which the column already says with
tier, presentation, silence, variant_id, and variant_choice. Whether it is
written at all is a
...
| The mini seen of a seen/saad khilaf composes into the saad | Where the script writes it, it is a unit with its own column, riding the base letter | The base owns and aligns the realization in both faces. The mini seen presents the seen face and becomes variant-silent in the saad face, preserving both column widths. Where the script writes no mark there is no riding column and the variant changes the base column's phoneme alone, which is Uthmani at 88:22:3 |
...
Four source-unit tokenizations section 7.2 names are now decided in the native
LetterUnit builder. Each keys on the typed grapheme kind and the fact it
supplies, never on the raw codepoint, and each is asserted by a native fixture:

| Decision | Closed as | How the builder decides it |
| --- | --- | --- |
...
| Mini seen as a riding unit | Its own letter unit wherever the script writes it, riding the base sound owner; it presents the seen realization or becomes variant-silent | A decorated tajweed_mark on a word with no sakt opens its own unit and names the base it rides in written_on_unit_id |
...
and its shadda straddling an independently tokenized haraka is attested.
