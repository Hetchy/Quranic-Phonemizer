# Hafs recitation overview

This is a domain-language synthesis of the Hafs reading represented by the
package. It connects the narrower research notes in this directory and states
the common terminology, boundary states, rule effects, and invariants in one
place. It is not a code architecture or a claim that every described nuance is
already exposed by the public API. Performance duration is not a phonemizer
output.

## Orientation

Tajweed is the set of phonological rules governing Quranic recitation. It
covers articulation, assimilation, nasalisation, vowel length, and the changes
caused by joining, stopping, and starting. This package currently implements
those rules for Hafs.

Arabic writes the consonantal skeleton with letters and normally writes short
vowels as marks:
...
| `ُ` | damma | short `u` |
| `ِ` | kasra | short `i` |
| `ْ` | sukun | no following vowel |
| `ّ` | shaddah | doubled consonant |

A matching carrier lengthens a short vowel: fatha plus alef gives long `a`,
damma plus waw gives long `u`, and kasra plus yaa gives long `i`. Dagger alef,
small waw, and small yaa can write the same long-vowel functions in compact
form. Tanween is a word-final short vowel followed by an `n` sound; its written
forms are fathatan, dammatan, and kasratan.
