...
| Initial U- and I-badal spellings projected a carrier consonant instead of a long qata. | Adapter defect | Project the reviewed initial-badal sequence as a long hamza nucleus and attach its written waw or yaa as the carrier. The 193-site A/U/I source register remains independently counted. |
...
| The first naql-modified badal row used a tanwin host, so ordinary moved-haraka hosts remained canonically voweled and never triggered naql. | Coverage and adapter defect | Added a separate A/U/I matrix for moved-haraka, tanwin, and ayah-edge joins; demote the host before an initial badal as for ordinary latent qata; and reconcile 190 sites as 186 within-ayah plus 4 ayah-edge overlaps. Three of the 193 initial-long source shapes belong to hamza meetings instead. |
| The latent `ا۟وْل...` spelling was grouped with genuine initial U-badal. | Script classification defect | Classify all 34 sukun-waw spellings with the 220 explicit `أُوْ...` forms: ibtidaa restores short `/ʔu/`, Naql transfers short `/u/`, and the rasm-only waw keeps orthographic silence without inheriting Naql. The 13 `ا۟و...` forms without waw sukun remain genuine U-badal. |
| The first matrix expected a transferred short vowel followed by the badal long vowel (`/a a:/`, `/u u:/`, or `/i i:/`). | Realisation and attribution defect | Keep one long vowel owned by the second-word carrier and merge the preceding host into it. The analysis view renders that shared sound as a cross-word bridge carrying both `naql` and `madd_badal`. |
