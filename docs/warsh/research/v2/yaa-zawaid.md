...
- among the 46 long-vowel sites, 10 precede hamzat qata and receive
  `madd_jaiz_munfasil`; the other 36 receive `madd_tabii`. Row 7 also
...
wasl                          ... dd a: ʕ i: ʔ i ð a:     madd_jaiz_munfasil on i:
ibtidaa at host and continue  ʔ a dd a: ʕ i: ʔ i ð a:    madd_jaiz_munfasil on i:
...
`/ʔ a: t a: n/`. The following article onset receives `wasl_elision` in the
...
| ---: | --- | --- | --- | --- |
| 1 | `2:185:9` | `2:186` | `اَ۬لدَّاعِۦٓ` | `/i:/`, `madd_jaiz_munfasil` |
...
| 8 | `17:62:8` | `17:62` | `اَخَّرْتَنِۦٓ` | `/i:/`, `madd_jaiz_munfasil` |
...
| 15 | `20:91:9` | `20:93` | `تَتَّبِعَنِۦٓ` | `/i:/`, `madd_jaiz_munfasil` |
...
| 23 | `35:26:7` | `35:26` | `نَكِيرِۦٓۖ` | `/i:/`, `madd_jaiz_munfasil` |
| 24 | `36:22:15` | `36:23` | `يُنقِذُونِۦٓۖ` | `/i:/`, `madd_jaiz_munfasil` |
...
The ten `madd_jaiz_munfasil` rows are 1, 8, 15, 23, 24, 31, 34, 37,
...
- `rules_on_sound()` places exactly one effective madd rule on `/i:/`:
  `madd_jaiz_munfasil` before qata, otherwise `madd_tabii`;
- row 7 additionally places the independent `madd_badal` origin occurrence on
  `/i:/`, its hamza, and its joined yaa source witnesses;
- `rules_on_char()` places that madd occurrence on the selected small-yaa and
  carrier/maddah witnesses. For munfasil it also reaches the following qata
  source unit as the causal participant; and
...
following article source independently receives `wasl_elision` when joined.
